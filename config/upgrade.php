...
//check to see if this is already installed, if so don't do anything
	if (  upgrade_needed() == false) {
		...
		_die("<h6>Already Upgraded</h6>
			   <small>Your Prosper202 version $version is already upgraded. <a href='".get_absolute_url()."login.php'>log in</a></small>" ); 	
...
//only show install setup, if it, of course, isn't installed already.
	...
	<?php } else if ($success == true) { unset($_SESSION['user_id']); ?>
		...
		<h6>Success!</h6>
		<small>Prosper202 has been upgraded! Now you can <a href="<?php echo get_absolute_url();?>account/signout.php">log in</a>.</small>
