...
try {
	...
	// Error handling
		...
		_die("<h6>Error establishing a database connection</h6>
			<p><small>This either means that the username and password information in your <code>config.php</code> file is incorrect or we can't contact the database server at <code>$dbhost</code>. This could mean your host's database server is down.</small></p>
			<small>
			<ul> 
...
if ($skip_upgrade == false) {
	...
	if (  is_installed() == true) {
		...
		if ((upgrade_needed() == true) and (($navigation[1] != 'config') and ($navigation[2] != 'upgrade.php'))) {
			header('location: '.get_absolute_url().'config/upgrade.php'); die();
		}	
	}
}
...
switch ($navigation[1]) { 
	...
	case "alerts202":
	case "stats202":
		if (@ini_get('safe_mode')) { header('location: '.get_absolute_url().'account/disable-safe-mode.php'); die();  }
		break;
}
