...
include_once(substr(dirname( __FILE__ ), 0,-9) . '/config/connect.php');
?>
...
$(document).on("submit", "#upgradeAlertApiKey", function (e) {
    ...
    $btn.button('loading');
    $.post("<?php echo get_absolute_url();?>account/ajax/upgrade_submit_api_key.php", $('#upgradeAlertApiKey').serialize(true))
		.done(function(data) {
		  	$btn.button('reset');
		  	var parsedJson = $.parseJSON(data);
		  //	console.log(parsedJson);
		  	if(parsedJson.error == false) {
		  		$.post("<?php echo get_absolute_url();?>account/ajax/upgrade_submit_api_key.php", {get_alert_body: true})
					.done(function(data) {
						$('#noKeyBody').html(data);
					  	$btn.button('reset');
			    });
...
$(document).on('change', 'select[name=rule_type]', function() {
	...
	switch(val) {
        ...
        case 'eu':
			select.tokenfield('destroy');
			select.attr('placeholder', 'Type true, false or Unknown and hit Enter');
			rotator_tags_autocomplete_truefalse(select_id, 'eu');
		break;
...
function rotator_tags_autocomplete_truefalse(selector){
	var elt = $('#' + selector);

	var tags = new Bloodhound({
	  local: [{value: 'true', label: 'True'}, {value: 'false', label: 'False'}, {value: 'Unknown', label: 'Unknown'}],
	  datumTokenizer: function(d) {
