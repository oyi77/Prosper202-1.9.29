$(document).ready(function() {
	...
	$("#get-logs").click(function() {
		...
		$.post("/account/ajax/conversion_logs.php", $('#logs_from').serialize(true))
		  .done(function(data) {
	...
	$('input[name=maxmind-isp]').on("change.radiocheck", function(){
        var checkbox = $(this);
        $.post("/account/administration.php", { maxmind: checkbox.val()})
		  .done(function(data) {
		  	if(data){
    ...
    $('#generate-new-api-key').click(function(){
    	...
    	$.post("/account/account.php", { add_rest_api_key: true, rest_api_key: key})
		  .done(function(data) {
		  	$("#no-api-keys").remove();
...
$(document).on('closed.bs.alert', '#prosper-alerts', function() {
    ...
    $.post("/account/ajax/alert-seen.php", { prosper_alert_id:id })
});

$(document).on('click', '#delete-rest-key', function() {
    var key = $(this).parent().attr("id");
    
    $.post("/account/account.php", { remove_rest_api_key: true, rest_api_key: key})
		.done(function(data) {
